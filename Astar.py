...
#change graph and coordinates according to Layout being used
...
def heuristic(node, end):
    return abs(coordinates[node][0] - coordinates[end][0]) + abs(coordinates[node][1] - coordinates[end][1])
...
def a_star_search(start, goal):
    ...
    # Dictionaries to store the cost from start and the parent nodes for path reconstruction
    g_cost = {start: 0}
    parents = {start: None}
    ...
    while open_list:
        ...
        # If we reached the goal, reconstruct and return the path and its cost
        if current_node == goal:
            ...
            path.reverse()
            return path, g_cost[goal]  # Return the path and the total cost
    ...
    return None, float('inf')  # No path found, returning infinity as cost


def for_all_combinations(tabels):
    all_combinations = list(itertools.permutations(tabels, len(tabels)))
    all_paths = []
    all_costs = []
    for array in all_combinations:
        array = array + ('A_1',) #change according to Layout being used
        privous = 'A_1'
        sum_cost = 0
        long_path = []

        for table in array:
            path, cost = a_star_search(privous, table)
            privous = table
            long_path.append(path)
            sum_cost += cost

        all_paths.append(long_path)
    ...
    # Get the row(s) with the minimum cost
    min_cost_row = df[df['Cost'] == min_cost]

    # Extract the path(s) corresponding to the minimum cost
    min_cost_path = min_cost_row['Path'].values[0]

    final_path =[]
    previous = None
    for array in min_cost_path:
        for element in array:
            # Add the element to the final path if it's not the same as the previous one
            if previous != element and previous is not None:
                final_path.append((previous, element))
